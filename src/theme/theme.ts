...
import { darkPalette, lightPalette, MONO_FONT } from "./tokens";
...
declare module "@mui/material/styles" {
	...
	interface PaletteOptions {
		...
		iconMuted: string;
		primaryHover: string;
		chips: Record<ChipColor, { bg: string; color: string }>
	  }
	  interface Palette {
		// gradient: string;
		// border: {
...
// const PRIMARY: Record<ThemeMode, string> = {
// 	light: "#1976d2",
// 	dark: "#7ab0e5",
// };
...
export const createAppTheme = (): Theme => {
	...
	primaryHover: "rgba(25,118,210,0.05)",
					chips: {
						[ChipColor.green]: { bg: "#e6f4ea", color: "#1e7e34" },
			...
			MuiButton: {
				...
				styleOverrides: {
					...
					contained: ({ theme }) => ({
						...
						"&:hover": { boxShadow: `0 2px 6px ${theme.vars.palette.primary.light}` },
